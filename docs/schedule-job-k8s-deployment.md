...
- Job 仍 Active，或集群 API 暂时不可达 → **不删 Job、不改执行记录**，等下一轮再看。Admin 已取消且 Job 仍 Active 的除外（仍 DELETE）。不补 Create
...
| P0-12 | Lease 过期按 Job 终态收尾；仍 Active 或 API 不可达则保留 Job，下一轮再收割 | `KubernetesCrashRecovery`、`ExecutionService::recoverRow` |
