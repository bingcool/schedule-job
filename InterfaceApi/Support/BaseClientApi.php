...
abstract class BaseClientApi
{
    ...
    /**
     * 解析 JSON 响应体为数组，非 2xx 或非法 JSON 抛 ClientException。
     *
     * @return array<string, mixed>
     */
    protected function parseJsonResponse(ResponseInterface $response): array
    {
        ...
        try {
            ...
            throw new ClientException('Invalid JSON: ' . $e->getMessage(), $response->getStatusCode(), $raw);
        }
        if (!is_array($decoded)) {
            throw new ClientException('Expected JSON object', $response->getStatusCode(), $decoded);
        }

        return $decoded;
    }

    /** 校验业务 code 字段，非 0 抛 ClientException */
    protected function assertBusinessOk(array $payload): void
    {
        ...
        if ($code !== 0 && $code !== '0') {
            ...
            throw new ClientException($msg, $status, $payload);
        }
    }
    ...
    protected function parseXmlResponse(ResponseInterface $response): array
    {
        ...
        try {
            ...
            if ($xml === false) {
                ...
                throw new ClientException('Invalid XML: ' . $message, $response->getStatusCode(), $raw);
            }
            ...
            throw new ClientException('Invalid XML: ' . $e->getMessage(), $response->getStatusCode(), $raw);
        } finally {
